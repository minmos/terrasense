...
static void sys_led_task(void *arg)
{
    ...
    while (1) {
        ...
        // --- 1. HANDLE OVERRIDE NOTIFICATIONS FIRST ---
        if (led_obj->override_blinks_left > 0) {
            ...
            if (override_tick == 0) {
                // Pass the stored struct directly
                sys_led_set_color(led_obj, led_obj->override_color, 30);
            } else if (override_tick == 2) {
        ...
        switch (led_obj->current_state) {
            case SYS_LED_STATE_BOOTING:
                if (state_changed || tick_count % 5 == 0) {
                    if (toggle_flag) sys_led_set_color(led_obj, LED_COLOR_YELLOW, 10);
            ...
            case SYS_LED_STATE_ERROR:
                if (state_changed || tick_count % 2 == 0) {
                    if (toggle_flag) sys_led_set_color(led_obj, LED_COLOR_RED, 50);
                    else sys_led_set_color(led_obj, LED_COLOR_BLACK, 0);
                    toggle_flag = !toggle_flag;
                }
                break;
            ...
            case SYS_LED_STATE_OK_DAY:
                if (state_changed) sys_led_set_color(led_obj, LED_COLOR_GREEN, 10);
                break;

            case SYS_LED_STATE_OK_NIGHT:
                if (state_changed) sys_led_set_color(led_obj, LED_COLOR_BLUE, 10);
                break;
...
esp_err_t sys_led_init(sys_debug_led_t *led_obj, gpio_num_t gpio_num, uint16_t max_leds)
{
    ...
    if (err == ESP_OK) {
        ...
        SYS_LOG("Debug LED hardware initialized on GPIO %d", gpio_num);
        xTaskCreate(sys_led_task, "sys_led_daemon", 2048, led_obj, 2, &led_obj->task_handle);
    }
    return err;
}

// Updated to unpack the struct
