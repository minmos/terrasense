...
void actor_fan_init(sys_debug_led_t *debug_led)
{
    ...
    for (int i = 0; i < HARDWARE_FAN_COUNT; i++) {
        ...
        if (HARDWARE_FAN_CONFIG[i].is_4pin) {
            // For a 4-pin fan, the MOSFET/power pin must be locked fully OPEN 
            ...
            gpio_set_level(HARDWARE_FAN_CONFIG[i].power_pin, 1); 
        ...
        // if 3pin, than we control over mosfet and power pin, if 4pin we control over extra pwm pin
...
bool actor_fan_set_speed(int index, uint8_t speed_percent)
{
    ...
    // calculating 8-bit duty cycle (0-255) using the MAPPED hardware target
    uint32_t duty = (hw_target * 255) / 100;

    // the 4pin fan we have needs Inversion
